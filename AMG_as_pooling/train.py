...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        assign_x = Variable(data['assign_feats'].float(), requires_grad=False).cuda()
        prol = Variable(data['prol'], requires_grad=False).cuda()
        prol = prol.float()

        ypred = model(h0, adj, batch_num_nodes, prol, assign_x=assign_x)
