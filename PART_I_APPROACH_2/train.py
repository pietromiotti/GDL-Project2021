...
def evaluate(dataset, model1, model2, pred_model, args, name='Validation', max_num_examples=None):
    model1.eval()
    model2.eval()
    labels = []
    preds = []
    for batch_idx, data in enumerate(dataset):
      adj1 = Variable(data['adj1'].float(), requires_grad=False).cuda()
      h01 = Variable(data['feats1'].float()).cuda()
      adj2 = Variable(data['adj2'].float(), requires_grad=False).cuda()
      h02 = Variable(data['feats2'].float()).cuda()

      labels.append(data['label'].long().numpy())
      batch_num_nodes1 = data['num_nodes1'].int().numpy()
      batch_num_nodes2 = data['num_nodes2'].int().numpy()
      assign_input1 = Variable(data['assign_feats1'].float(), requires_grad=False).cuda()
      assign_input2 = Variable(data['assign_feats2'].float(), requires_grad=False).cuda()

      y1 = model1(h01, adj1, batch_num_nodes1, assign_x=assign_input1)
      y1 = y1.cuda()
      y2 = model2(h02, adj2, batch_num_nodes2, assign_x=assign_input2)
      y2 = y2.cuda()
      y3 = torch.cat((y1,y2),1)
      y3 = y3.cuda()
      ypred = pred_model(y3).cuda()
      ypred= ypred.cuda()
...
def train(dataset, model1, model2, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
        mask_nodes = True):

    pred_model = model1.build_pred_layers(2*model1.pred_input_dim * (model1.num_pooling+1), [50], args.num_classes, 1).cuda()

    optimizer = torch.optim.Adam(list(filter(lambda p : p.requires_grad, model1.parameters()))+list(filter(lambda p : p.requires_grad, model2.parameters())), lr=0.001)
    iter = 0
    best_val_result = {
            'epoch': 0,
            'loss': 0,
    ...
    #in both training and validation we needed to get adjacencies and feature vectors of both graph
    for epoch in range(args.num_epochs):
        ...
        print('Epoch: ', epoch)
        for batch_idx, data in enumerate(dataset):
            #Get all the data from the batched graph
            begin_time = time.time()
            model1.zero_grad()
            model2.zero_grad()
            adj1 = Variable(data['adj1'].float(), requires_grad=False).cuda()
            h01 = Variable(data['feats1'].float(), requires_grad=False).cuda()
            adj2 = Variable(data['adj2'].float(), requires_grad=False).cuda()
            h02 = Variable(data['feats2'].float(), requires_grad=False).cuda()
            label = Variable(data['label'].long()).cuda()
            batch_num_nodes1 = data['num_nodes1'].int().numpy() if mask_nodes else None
            batch_num_nodes2 = data['num_nodes2'].int().numpy() if mask_nodes else None
            assign_input1 = Variable(data['assign_feats1'].float(), requires_grad=False).cuda()
            assign_input2 = Variable(data['assign_feats2'].float(), requires_grad=False).cuda()

            #Soft Assign Original Graph
            y1 = model1(h01, adj1, batch_num_nodes1, assign_x=assign_input1)
            y1 = y1.cuda()

            #Soft Assign Coarsened Graph
            y2 = model2(h02, adj2, batch_num_nodes2, assign_x=assign_input2)
            y2 = y2.cuda()
            ...
            #Please consider that we modified the total loss since we used 2 Soft-Assign GNNs and a Classical Neural Network for classsification purpose
            loss = F.cross_entropy(ypred, label, reduction='mean') + \
                model1.loss(ypred, label, adj1, batch_num_nodes1) + model2.loss(ypred, label, adj2, batch_num_nodes2)

            loss.backward()

            nn.utils.clip_grad_norm_(model1.parameters(), args.clip)
            nn.utils.clip_grad_norm_(model2.parameters(), args.clip)
        ...
        result = evaluate(dataset, model1, model2, pred_model, args, name='Train', max_num_examples=100)
        train_accs.append(result['acc'])
        train_epochs.append(epoch)
        if val_dataset is not None:
            val_result = evaluate(val_dataset, model1, model2, pred_model, args, name='Validation')
        ...
        if test_dataset is not None:
            test_result = evaluate(test_dataset, model1, model2, pred_model, args, name='Test')
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    ...
    for i in range(10):
        ...
        if args.method == 'soft-assign':
            print('Method: soft-assign')
            model1 = encoders.SoftPoolingGcnEncoder(
                    max_num_nodes,
                    input_dim, args.hidden_dim, args.output_dim, args.num_classes, args.num_gc_layers,
                    args.hidden_dim, assign_ratio=args.assign_ratio, num_pooling=args.num_pool,
                    bn=args.bn, dropout=args.dropout, linkpred=args.linkpred, args=args,
                    assign_input_dim=assign_input_dim).cuda()
            model2 = encoders.SoftPoolingGcnEncoder(
                    max_num_nodes,
                    input_dim, args.hidden_dim, args.output_dim, args.num_classes, args.num_gc_layers,
                    args.hidden_dim, assign_ratio=args.assign_ratio, num_pooling=args.num_pool,
                    bn=args.bn, dropout=args.dropout, linkpred=args.linkpred, args=args,
        ...
        _, val_accs = train(train_dataset, model1, model2, args, val_dataset=val_dataset, test_dataset=None,
            writer=writer)
