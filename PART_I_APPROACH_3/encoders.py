...
from easyAMG import computeLaplacian, computePrologator
...
class AmgPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        super(AmgPoolingGcnEncoder, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim,
        ...
        #ONLY 1 AMG POOLING IS COMPUTED
        num_pooling = 1;
    ...
    def forward(self, x, adj, batch_num_nodes, prol, **kwargs):
        ...
        degree = adj.sum(2);
        adj = -adj;
        adj[:, range(adj.shape[1]), range(adj.shape[2])] = degree

        #Restrict the laplacian
        adj = torch.transpose(prol, 1, 2) @ adj @ prol

        #Compute back the new Adjacency Matrix
        adj[:, range(adj.shape[1]), range(adj.shape[2])] = 0
        adj = -adj;

        x_a = x

        #Compute new embedding using the new Adjancency Matrix
        embedding_tensor = self.gcn_forward(x, adj,
                self.conv_first_after_pool[0], self.conv_block_after_pool[0],
                self.conv_last_after_pool[0])
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        '''
        eps = 1e-7
        loss = super(AmgPoolingGcnEncoder, self).loss(pred, label)
