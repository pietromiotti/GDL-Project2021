...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            ...
            assign_input_dim=-1, args=None, goren_loss_type=0):
        ...
        self.goren_loss_type = goren_loss_type
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        for i in range(self.num_pooling):
            ...
            self.assign_tensor = nn.Softmax(dim=-1)(self.assign_pred_modules[i](self.assign_tensor)/self.temperature)
            if embedding_mask is not None:
    ...
    def loss(self, pred, label, adj=None, laplacian=None, evs=None, batch_num_nodes=None):
        ...
        if self.goren_loss_type == 0:
            p = util.projection0(self.assign_tensor)
            newlaplacian = util.batch_comb_laplacian_torch(torch.transpose(self.assign_tensor, 1, 2)@adj@self.assign_tensor)
            for i in range(evs.size()[0]):
                for j in  range(evs.size()[1]):
                    goren_loss += torch.abs(util.QF(laplacian[i],evs[i][j])-util.QF(newlaplacian[i],(p[i]@evs[i][j])))/evs.size()[1]

        elif self.goren_loss_type == 1:
            p, Gamma = util.projection1(self.assign_tensor)
            newlaplacian = (Gamma@util.batch_comb_laplacian_torch(torch.transpose(self.assign_tensor, 1, 2)@adj@self.assign_tensor)@Gamma)
            for i in range(evs.size()[0]):
                for j in  range(evs.size()[1]):
                    # use QF -- envs are l2 normalized
                    goren_loss += torch.abs(util.QF(laplacian[i],evs[i][j])-util.RQ(newlaplacian[i],(p[i]@evs[i][j])))/evs.size()[1]

        loss = goren_loss + loss + self.link_loss
