...
class GraphSampler(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, G_list, args, features='default', normalize=True, assign_feat='default', max_num_nodes=0):
        self.adj_all = []
        self.len_all = []
        self.feature_all = []
        self.label_all = []

        self.noeigen = args.neigen
        self.args = args
    ...
    def __getitem__(self, idx):
        ...
        L = util.comb_laplacian_np(adj)

        laplacian = np.zeros((self.max_num_nodes, self.max_num_nodes),dtype=float)
        laplacian[:num_nodes, :num_nodes] = L

        _, eigenv = np.linalg.eigh(L)
        eigenv_padded = np.zeros((self.noeigen, self.max_num_nodes),dtype=float)
        eigenv_padded[:min(num_nodes,self.noeigen),:num_nodes] = eigenv[:,:min(num_nodes,self.noeigen)].transpose()
